...
func (cm *ConnectionManager) GetIOContext() (*rados.IOContext, error) {
	...
	if err != nil {
		cm.logger.Error("failed to open IO context", "error", err)
...
func (cm *ConnectionManager) GetPoolByName(poolName string) (int64, error) {
	cm.mu.RLock()
	conn := cm.conn
	cm.mu.RUnlock()
...
func (cm *ConnectionManager) GetConfigOption(option string) (string, error) {
	cm.mu.RLock()
	conn := cm.conn
	cm.mu.RUnlock()
